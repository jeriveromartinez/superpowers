...
**Architecture:** Three Markdown profiles are bundled with the fork. A dependency-free Node installer copies them only when the operator explicitly supplies an OpenCode configuration directory. The existing V2 bootstrap receives the role-selection policy and safe `general` fallback; V1 remains unchanged.
...
- Use exactly: `zai-org/GLM-5.3`, `z-ai/glm-5.3-flash`, `xiaomi/mimo-v2.5`.
...
| Path | Responsibility |
| --- | --- |
| `opencode/model-routing/agents/superpowers-*.md` | Three V2 profile definitions. |
...
**Interfaces:**
- Consumes: `--config-dir <path>` and three package profile files.
...
**Interfaces:**
- Consumes: the three Task 1 IDs.
- Produces: `V2_MAPPING` role guidance; V1 still uses `task` with `subagent_type: "general"`.

- [ ] **Step 1: Add failing mapping assertions**

Import `.opencode/plugins/superpowers.js` by `pathToFileURL`. Require `V2_MAPPING` to include `superpowers-expert`, `superpowers-main`, `superpowers-economic`, `architecture`, `implementation`, `exploration`, and `general`. Require `V1_MAPPING` to include `` `task` with `subagent_type: "general"` `` and not include `superpowers-expert`.
...
Extend the Node test to read both guides. Require each to contain `install-opencode-model-routing.mjs`, `opencode models`, `superpowers-expert`, `superpowers-economic`, `zai-org/GLM-5.3`, and `xiaomi/mimo-v2.5`. Also require `does not modify` beside `opencode.jsonc`.
...
Add `Model routing (OpenCode V2)` to both files. Include the PowerShell command `node .\scripts\install-opencode-model-routing.mjs --config-dir "$HOME\.config\opencode"` and POSIX command `node ./scripts/install-opencode-model-routing.mjs --config-dir "$HOME/.config/opencode"`, each followed by `opencode models`. List all three roles and model IDs. State that providers must enable models, the installer only copies profiles and does not modify `opencode.json` or `opencode.jsonc`, collisions are refused, missing profiles fall back to `general`, and only the three copied `agents/superpowers-*.md` files should be removed to uninstall.
...
Expected: catalog lists the three IDs and the session can select `superpowers-economic`. If OpenCode, credentials, or a model is unavailable, report this as unperformed, not passed.

- [ ] **Step 4: Deliver**

Report the installer command, all three mappings, static suite result, and separate runtime result.
